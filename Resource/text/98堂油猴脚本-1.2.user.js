...
(async function () {
    ...
    function getSettings() {
        ...
        return {
            ...
            showQuickGrade: GM_getValue("showQuickGrade", true), // 是否显示手动评分
            showAutoGrade: GM_getValue("showAutoGrade", true), // 是否显示手动评分
    ...
    function addStyles() {
        ...
        style.innerHTML = `
            ...
            /* --- 通用自定义按钮的样式 --- */
            .bgsh-customBtn, .bgsh-searchBtn, .bgsh-quickTopicadminToPostBtn, .bgsh-quickReplyToPostBtn,.bgsh-QuickMiscReportBtn,.bgsh-quickReportadToPostBtn,.bgsh-quickGradeToPostBtn ,.bgsh-openAllUrlBtn,.bgsh-fastPMButtonBtn,.bgsh-quickReplyEditToPostBtn,.bgsh-setAnswerToPostBtn,.bgsh-QuickSupportBtn {
              padding: 8px 15px;
            ...
            .bgsh-quickTopicadminToPostBtn ,.bgsh-quickReplyToPostBtn,.bgsh-quickReplyEditToPostBtn,.bgsh-setAnswerToPostBtn.bgsh-QuickSupportBtn{
              width: auto;
              float: right;
            ...
            /* 按钮的悬停效果 */
            .bgsh-quickTopicadminToPostBtn:hover ,.bgsh-quickReplyToPostBtn:hover ,.bgsh-QuickMiscReportBtn:hover,.bgsh-quickReportadToPostBtn:hover,.bgsh-fastPMButtonBtn:hover,.bgsh-quickReplyEditToPostBtn:hover,.bgsh-setAnswerToPostBtn:hover,.bgsh-QuickSupportBtn:hover{
              transform: scale(1.05);
    ...
    /**
     * 注册一个命令到Tampermonkey的上下文菜单，允许用户访问设置界面。
     */
            ...
            <div class='bgsh-setting-box'>
                <div class='bgsh-setting-box-container '>
                    <div class="bgsh-setting-first">
                        ...
                        <label for="tipsText">提示文字（尽自己可见）</label>
                        <input type="text" id="tipsTextInput" value="${
                            settings.tipsText
                        }">
                        <br>
                        <label for="logoText">自动评分消息（给楼主评分后楼主会收到）</label>
                        ...
                        <br>
                        <label class='bgsh-setting-checkbox-label' for="showAutoGradeCheckbox">
                        <input type='checkbox' id="showAutoGradeCheckbox" ${
                            settings.showAutoGrade ? "checked" : ""
                        }>
                            <span class='bgsh-setting-checkbox-custom'></span>
                            <span>启用自动评分</span>
                        </label>
    ...
    function stylePosts(settings) {
        ...
        images.forEach((image) => {
            ...
            if (link) {
                // 在 link 的父元素中查找 id 为 "buyinfouniqueSpanId" 的元素
    ...
    */
                ...
                if (buyInfo.state) {
                    ...
                    if (link) {
                        ...
                        if (!existingSpan) {
                            ...
                            span.id = "buyinfouniqueSpanId"; // 设置 span 的 ID
                            span.style.cssText =
    ...
    /**
     * 创建“自动评分”按钮，用于页面内对主帖的内容快速评分
     * @return {HTMLElement} 按钮元素
     */
    function createAutoGradeButton(pid) {
        console.log("error"+pid)
        return createButton("autoGradeButton", "自动评分", autoGrade);
    }

    /**
     * 创建“手动评分”按钮，用于页面内对主帖的内容手动评分
     * @return {HTMLElement} 按钮元素
     */
    function createQuickGradeButton(tid, pid) {
        return createButton("quickGradeButton", "手动评分", () =>
            gradeManual(tid, pid)
        );
    }
    ...
    /**
     * 添加对回复进行操作的按钮，用于页面内对回复的内容楼层置顶/回复等等
     * @return {HTMLElement} 按钮元素
     */
    ...
    function handlePostPage(settings, buttonContainer) {
        ...
        if (userid) {
            ...
            }if (settings.showAutoGrade) {
                const tid = extractTid(window.location.href);
                buttonContainer.appendChild(createAutoGradeButton(tid, pid));
            }
    ...
    function saveSettings(settings) {
        ...
        settings.showAutoGrade = document.getElementById(
            "showAutoGradeCheckbox"
        ).checked;
        ...
        const settingsToSave = {
            ...
            showAutoGrade: settings.showAutoGrade,
        ...
        if (
            ...
            oldSettings.showAutoGrade !== settings.showAutoGrade ||
